...
double week_num_thres=1.0; // could filter out people with too short observation
double nw_thres=1.0; // could filter out people who travel infrequently
bool has_work;
...
void process_single_user(const std::vector<StayRegion>& regions, 
    ...
    for (const auto& region : sorted_regions) {
        if (!region.home_h3_index.empty()) {
            ...
            if (stringToH3(home_h3.c_str(), &h3Index) == 0) {
                ...
                if (cellToLatLng(h3Index, &latLng) == 0) {
                    // Use H3's built-in degsToRads function
                    home_lat = convertRadsToDegrees(latLng.lat);  // Use H3's function
                    home_lon = convertRadsToDegrees(latLng.lng);  // Use H3's function
                }
            }
        }
        if (!region.work_h3_index.empty()) {
            ...
            if (stringToH3(work_h3.c_str(), &h3Index) == 0) {
                ...
                if (cellToLatLng(h3Index, &latLng) == 0) {
                    // Use H3's built-in degsToRads function
                    work_lat = convertRadsToDegrees(latLng.lat);  // Use H3's function
                    work_lon = convertRadsToDegrees(latLng.lng);  // Use H3's function
                }
            }
        }
    }
    ...
    for (size_t i = 0; i < simu_time.size(); i++) {
        ...
        if (simu_loc[i] == 1) { // Home
            ...
            // Generate a unique ID for other locations
            simu_locid.push_back(i + 3); // Start at 3 to avoid conflict with home (1) and work (2)
            
            // Generate location near home or work with some variance
            if (rand() % 2 == 0) {
    ...
    // std::cout << "Writing user " << regions[0].user_id << " home coords: " << home_lat << "," << home_lon 
    //           << " work coords: " << work_lat << "," << work_lon << std::endl;
...
void process_users_batch(std::unordered_map<std::string, std::vector<StayRegion>>& user_data,
    ...
    for (auto& [user_id, regions] : user_data) {
        ...
        process_single_user(regions, fout_id1, fout_id2, fout_id3, fout_id4, fout_id5, fout_id6, fout_id7);
    }
...
void process_stay_regions_streaming(const std::string& input_path, 
                                   ...
                                   FILE* fout_id4, FILE* fout_id5, FILE* fout_id6, FILE* fout_id7) {
    // Open the file
    auto maybe_infile = arrow::io::ReadableFile::Open(input_path, arrow::default_memory_pool());
    if (!maybe_infile.ok()) {
    ...
    if (!maybe_reader.ok()) {
        std::cerr << "Could not open Parquet file: " << input_path << std::endl;
        return;
    }
            ...
            for (int c = 0; c < num_chunks; c++) {
                ...
                for (int64_t i = 0; i < chunk_size; i++) {
                    ...
                    if (user_data.size() >= CHUNK_SIZE) {
                        ...
                        process_users_batch(user_data, fout_id1, fout_id2, fout_id3, fout_id4, fout_id5, fout_id6, fout_id7);
                        user_data.clear();
                    }
                }
            }
...
void process_streaming(const std::string& input_path, bool commuter_mode) {
    ...
    if (commuter_mode) {
        file_name1 = "./Commuters/DTRealCommuters.txt";
        file_name2 = "./Commuters/DTSimuCommuters.txt";
        file_name3 = "./Commuters/DNRealCommuters.txt";
        file_name4 = "./Commuters/DNSimuCommuters.txt";
        file_name5 = "./Commuters/SimuLocCommuters.txt";
        file_name6 = "./Commuters/ParametersCommuters.txt";
        file_name7 = "./Commuters/RealLocCommuters.txt";
    } else {
        file_name1 = "./NonCommuters/DTRealNonCommuters.txt";
        file_name2 = "./NonCommuters/DTSimuNonCommuters.txt";
        file_name3 = "./NonCommuters/DNRealNonCommuters.txt";
        file_name4 = "./NonCommuters/DNSimuNonCommuters.txt";
        file_name5 = "./NonCommuters/SimuLocNonCommuters.txt";
        file_name6 = "./NonCommuters/ParametersNonCommuters.txt";
        file_name7 = "./NonCommuters/RealLocNonCommuters.txt";
    }
    ...
    if (!fout_id1 || !fout_id2 || !fout_id3 || !fout_id4 || !fout_id5 || !fout_id6 || !fout_id7) {
    ...
    for (const auto& file : parquet_files) {
        ...
        process_stay_regions_streaming(file, fout_id1, fout_id2, fout_id3, fout_id4, fout_id5, fout_id6, fout_id7);
    }
...
int main(int argc, char* argv[]) {
    ...
    // Default file path
    std::string input_path = "/home/aparimit/timegeo_code/new_codebase/work_locations.parquet";
    ...
    // Check if commuter mode is specified
    if (argc > 2) {
        commuter_mode = (std::string(argv[2]) == "true" || std::string(argv[2]) == "1");
    }
    
    std::cout << "Processing in " << (commuter_mode ? "commuter" : "non-commuter") << " mode" << std::endl;
    std::cout << "Reading from: " << input_path << std::endl;
    
    // Create output directories if they don't exist
    if (system("mkdir -p ./Commuters") != 0) {
        std::cerr << "Warning: Could not create Commuters directory" << std::endl;
    }
    if (system("mkdir -p ./NonCommuters") != 0) {
        std::cerr << "Warning: Could not create NonCommuters directory" << std::endl;
    }
    
    // Process the data
    process_streaming(input_path, commuter_mode);
